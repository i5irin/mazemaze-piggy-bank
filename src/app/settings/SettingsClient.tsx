...
export function SettingsClient() {
  ...
  const renderProviderSignInButton = (
    ...
    return (
      <ProviderSignInButton
        ...
        disabled={isDisabled || !data.isOnline}
        onSignIn={() => void handleProviderSignIn(providerId, options)}
      />
    );
  };
  ...
  const renderSignInStorageBody = () => {
    ...
    const connectedProviderId = activeProviderId;
    ...
    return (
      <>
        ...
        {connectedProvider.error ? <p role="status">{connectedProvider.error}</p> : null}
        {switchState.message ? (
          <div
            className={`app-alert ${switchState.status === "error" ? "app-alert-error" : ""}`}
            role="status"
